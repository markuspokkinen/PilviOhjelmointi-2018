...
exports.apiSearch = (bookId, callback) => {
	//console.log(bookId);
	if (bookId.length !== 12) {
		throw ('bookId should be 12 character long');
	} else {
		const url = 'https://www.googleapis.com/books/v1/volumes';
		const query_string = { q: bookId };
		request.get({ url: url, qs: query_string }, (err, res, body) => {
			if (err) {
				callback(new Error('error making google books request'));
			}
...
function validateBookId(BookID, callback) {
	const url = 'https://www.googleapis.com/books/v1/volumes';
	const query_string = { q: BookID };
	request.get({ url: url, qs: query_string }, (err, res, body) => {
...
exports.add = bookId => {
	...
	var valbook;
	validateBookId(bookId, (res) => { valbook = res; });
...
exports.remove = query => {
	...
	const ind = bookList.indexOf(query);
	if (ind !== -1) {
		bookList.splice(ind, 1);
		return 'book ' + query + ' removed';
	} else {
		throw ('no books found matching search');
	}
}
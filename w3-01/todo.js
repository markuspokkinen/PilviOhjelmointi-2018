...
do {
	...
	var input = String(readline.question('enter command: ')).toLowerCase()
	//const input = String(readline.question('enter command: '))
	//let input = String(readline.question('enter command: '))
	...
	if (input.indexOf('add ') === 0) {
		...
		if (items.indexOf(item) === -1) {
			...
			items.unshift(item)
			//items are added to the first index
		} else {
			console.log('array already contains item')
		}
	}
	...
	if (input.indexOf('remove') === 0) {
		...
		if (ind !== -1) {
			...
			console.log('array doesnt contain item')
		}
